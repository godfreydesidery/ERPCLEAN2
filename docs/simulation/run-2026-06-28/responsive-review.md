...
| Bakari Mbaga | Group Dashboard | mobile + tablet | **RESOLVED — was a wiring/discoverability gap, not a missing feature.** A real BI dashboard already exists at `/admin/dashboard` (guarded by `BI.VIEW`, renders Revenue/OpEx/Net-Profit/working-capital from `BiDashboardController`), and the GM's role holds `BI.VIEW` — but normal users landed on the placeholder `admin-home` and were never shown it. Fixed: `/admin/home` is now a permission-gated **launchpad** — 15 quick-launch cards, **Dashboard first**, each gated on the *same* permission its route guard requires (15/15 parity-checked, so a visible card is always navigable — also closes the silent-home-redirect class). Verified by re-capturing the GM's home: the Dashboard card is front-and-centre. Root keeps its setup grid; zero-permission users keep the calm placeholder. |
