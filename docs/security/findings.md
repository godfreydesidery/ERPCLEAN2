...
| F10 | 6 | MEDIUM | **Audit read is org-wide** — any `AUDIT.VIEW` holder sees all rows incl. other companies'. Acceptable ONLY while `AUDIT.VIEW` is root-only (it's on no non-root role today; root bypasses scope by design). | OPEN | Before granting `AUDIT.VIEW` to any company-scoped role, add a `company_id = activeCompany` predicate for non-root callers in `AuditReadService.search` (root stays org-wide). |
