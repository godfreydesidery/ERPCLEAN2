...
GL was built dimension-ready (ADR-0013 D-7): `journal_lines.branch_id` is a *nullable analysis tag*, never a separate ledger; a branch P&L is `journal_lines GROUP BY branch_id × account type`. The branch is the **first dimension**. What is missing — and what gates **Budgeting** (§3.14) — is a **generic, configurable dimension model** (Cost Centre, Department, …) and the **seam** that lands those dimension values on the journal lines and slices the books by them. **[Amended 2026-06-11 (Wave-2 collision #4): the original also listed Projects (§3.13) as gating on this framework. Projects (ADR-0033) ships its OWN dedicated `journal_lines.project_id` scalar tag and does NOT gate on or consume this framework's slots; only Budgeting (ADR-0034) hard-gates on it.]** The forces:
...
- **Budgeting builds on this — design its contract up front (FR-CC-17, NFR-CC-06).** Budgeting (ADR-0034) is greenfield and **hard-gates** on this framework: it attaches budget lines to cost-centre `dimension_values` and reads cost-centre actuals via this framework's `actualsByAccountValuePeriod` contract. The framework must expose a documented service/DTO contract (resolve value by uid, list active values, read actuals grouped by account×value×period) so it consumes the seam, not the tables. Resolved in **D-8**. **[Amended 2026-06-11 (Wave-2 collision #4): the original also claimed "Project is *one more dimension*, not a bespoke `project_id` scattered across postings." That is NOT how Projects shipped — ADR-0033 (OQ-PROJ-01) uses a DEDICATED `journal_lines.project_id` scalar tag (shaped to fold into this framework later), NOT a dimension slot. So Projects does not consume this framework's slots; only Budgeting hard-gates on it. A future reader must not wire Project into `dimension3`/`dimension4`.]**
...
> v1 seeds exactly two rows per company: `(COST_CENTRE, 'COST_CENTRE', 'Cost Centre', is_built_in=true)` and `(DEPARTMENT, 'DEPARTMENT', 'Department', is_built_in=true)`. `DIMENSION_3`/`DIMENSION_4` slots exist in the enum/CHECK but **no dimension row occupies them in v1** — they are **GENERIC-reserved** for two future user-defined dimensions, added as a `dimensions` INSERT + per-poster wiring, no schema change (BR-CC-02, NFR-CC-06, OQ-CC-08). **[Amended 2026-06-11 (Wave-2 collision #4): was "reserved for Project (§3.13) + one future"; the Project reservation is DROPPED — Projects (ADR-0033) uses a dedicated `journal_lines.project_id` scalar tag, not a dimension slot. Both reserved slots are now generic.]**
...
| `dimension3_value_id` | BIGINT | YES | NULL | FK → `dimension_values(id)`; **GENERIC-reserved** (a future user-defined dimension); `fk_journal_line_dimension3`; **no v1 poster writes it**. **[Amended 2026-06-11 (Wave-2 collision resolution #4): the "(Project, §3.13)" reservation is DROPPED — Projects (ADR-0033) uses its OWN dedicated `journal_lines.project_id` scalar tag, NOT this slot. This slot stays generic so a future reader does not wire project into dimension3.]** |
...
**`V29__cost_centre_dimension3_indexes.sql`** *(optional / forward — may be folded into V27)* — reserved for the partial indexes on `dimension3_value_id`/`dimension4_value_id` when a **future module activates a generic slot** (additive then). **v1 may ship V27+V28 only** and leave V29 for that future increment; the range is reserved either way. **[Amended 2026-06-11 (Wave-2 collision #4): was "when the Projects module activates slot 3 … V29 is the Projects module's to claim". Projects (ADR-0033) does NOT use this slot — it uses its own `journal_lines.project_id`. V29 is reserved for whichever future module first activates a generic slot, not earmarked for Projects.]**
...
**Positive**
- **Budgeting is unblocked** with a clean, documented contract (D-8 / FR-CC-17): Budgeting (ADR-0034) attaches budget lines to cost-centre dimension values and reads actuals via `actualsByAccountValuePeriod`. The two generic reserved slots (`dimension3`/`dimension4`) let a **future** module add a user-defined dimension (a `dimensions` INSERT + per-poster wiring) with no schema change (NFR-CC-06, OQ-CC-08). **[Amended 2026-06-11 (Wave-2 collision #4): the original cited Projects as the slot-3 consumer ("Projects adds a Project dimension in the reserved slot 3 … rather than scattering `project_id` columns"). Projects (ADR-0033) instead deliberately uses its OWN dedicated `journal_lines.project_id` scalar tag (its OQ-PROJ-01, shaped to fold into a future generic framework). So this framework does NOT host the Project dimension; the slots remain generic.]**
...
- **Two reserved slots (`dimension3/4`) carry nullable columns no v1 poster writes.** The cost is two NULL columns + (deferred) partial indexes — negligible, and the alternative (ALTER `journal_lines` again when a future dimension lands) is the migration the reserved slots avoid. **[Amended 2026-06-11 (Wave-2 collision #4): was "when Projects lands" — Projects uses its own `project_id` column, so these slots are held for a future generic dimension, not Projects.]**

**Neutral / deferred**
- Budgets, allocations, profit-centre roll-up dashboards, per-invoice-line dimension, full poster wiring, per-dimension security, dimension combination rules, FX dimension reporting — all deferred (spec §2), none precluded (NFR-CC-06). **[Amended 2026-06-11 (Wave-2 collision #4): the original said "The Project dimension is reserved (slot 3), built by Projects." Projects (ADR-0033) instead uses a dedicated `journal_lines.project_id` scalar tag — it does NOT consume a slot here. The two generic slots remain available for a future user-defined dimension.]**

## Alternatives considered

- **Dimension tag — fixed slot-columns (chosen) vs a `journal_line_dimensions` child table (EAV).** *Decided: fixed columns (D-3).* The child table allows unbounded dimensions but adds a join + N rows per posted line on the hot path and a heavier GROUP BY; the business reality is a small bounded set of dimensions (Cost Centre, Department, + ~2 future generic — note: Project is NOT one of these slots; ADR-0033 uses its own `project_id` tag, Wave-2 amendment #4) for which fixed nullable columns — the `branch_id` precedent — are faster to write, index (partial), and group. The EAV's flexibility is not worth the hot-path cost; if the business ever genuinely needs >4 simultaneous dimensions, the child table is an additive reshape under a new ADR. Fixed columns win for v1.
...
- **Project — a dimension in this framework vs a dedicated `project_id` column.** **[Amended 2026-06-11 (Wave-2 collision #4): this framework's slots stay GENERIC; Projects (ADR-0033) chose its OWN dedicated `journal_lines.project_id` scalar tag.]** This ADR originally reserved slot 3 for Project, reasoning that a project is "one more dimension" reusing the slot-columns/resolver/reports. **Projects (ADR-0033 / OQ-PROJ-01) instead deliberately ships a dedicated nullable `journal_lines.project_id` (+ `project_task_id` + `project_cost_type`) scalar tag** — its load-bearing fork, because this generic framework was not yet built when Projects was designed, and a project tag also rides `supplier_bill_lines`/`sales_invoices`/`stock_movements` for drill-down beyond what a single GL slot expresses. ADR-0033's column is explicitly shaped to fold into this framework later (a future framework can read/migrate `journal_lines.project_id` into a slot additively). **Net: this framework does NOT host Project; `dimension3`/`dimension4` remain generic-reserved for future user-defined dimensions.** A future reader must not wire Project into a dimension slot — Projects has its own tag.
...
- **OQ-CC-08 — Project dimension:** **[Amended 2026-06-11 (Wave-2 collision #4): was "reserve slot 3, built by Projects". Projects (ADR-0033) uses a dedicated `journal_lines.project_id` scalar tag, NOT a slot here. `dimension3`/`dimension4` stay GENERIC-reserved for future user-defined dimensions; neither is earmarked for Project.]** Settled.
