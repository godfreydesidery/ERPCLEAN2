...
| 4 | **Tier-1 Accounting** (GL / AR / AP / Cash&Bank / VAT / FX / year-end) | **PARTIAL** | XXL | GL/AR/AP **DONE**; Cash&Bank, VAT return, year-end, FX **remaining** |
...
- [ ] **Tax / VAT return (V14, ADR-0017)** — **L** — `vat_return_periods`, `vat_return_lines`, `vat_adjustments`, summary JSONB; `TaxReturnService.compute` aggregates output VAT (sales `tax_summary` per band) + input VAT (purchase bills). *Depends on AP input-VAT data (bill `vat_amount` captured; purchase-VAT line modelling is the open slice). TRA EFD/VFD fiscalisation stays deferred (OQ-SALES-03).*
- [~] **Year-end close & opening carry-forward** — **M** — periods open/close works; `YearEndCloseService` to auto-post P&L→Retained-Earnings closing entry + carry-forward opening balances + open next FY is missing (OQ-GL-03). *Essential for multi-year deployments.*
