...
## Finding #22 — BI / Analytics & Dashboards (ADR-0037) adversarial review batch (2026-06-13)

Multi-agent adversarial review of the Wave-2 BI/analytics + dashboard + FE-polish increment
(read-only com.erp.modules.bi aggregator, /admin/dashboard, axe a11y gate + Playwright + spec-deepening;
migration V81) — 5 dimensions, each finding adversarially verified (0 refuted). **16 findings → 2 HIGH +
3 MEDIUM confirmed (all FIXED), the rest LOW/quality.** The green suite (backend 844, web 436) had missed
the 2 HIGH. Full suite green after fixes: BI ITs 10/10, web ng test 438/438 (3× stable).

| # | Sev | Status | Issue → Fix |
|---|-----|--------|-------------|
| 22a | HIGH | **FIXED** | **Composite `/dashboard` bypassed the fine-grained perm gate** — gated only `@perm.has('BI.VIEW')` while the per-panel endpoints correctly add `BI.FINANCE/OPS/CRM.VIEW`. So a BI.VIEW-only holder read finance + AR/AP + inventory + CRM data (least-privilege / info-disclosure). Fix: `DashboardServiceImpl.dashboard()` now NULLs each panel the caller lacks the fine perm for (PermissionResolver check); BiExportFlattener already skips null panels so `/dashboard/export` auto-scopes. BI.VIEW stays the coarse route gate. IT proves a BI.VIEW-only principal gets null finance/inventory/crm panels. |
| 22b | HIGH | **FIXED** | **COGS hardcoded to ZERO → gross-profit==revenue, gross-margin==100% always**, diverging from the income statement (breaks the KPI==statement property the design rests on). The 5-type CoA can't separate COGS by account-type (the income statement uses StatementClassifier code-bands). Fix: dropped the fabricated `cogs`/`grossProfit`/`grossMarginPct` fields from FinanceSummaryDto (+ FE model/html/spec); kept revenue/opex/netProfit which equal AccountMovementQuery by construction. A missing metric beats a wrong one; the income statement remains the authoritative gross-profit surface. |
| 22c | MEDIUM | **FIXED** | **CRM panel returned ZERO on a null branchId** (composite dashboard called with branchId=null) — the same null-id-returns-zero class as the killed AR/AP ageing bug. Fix: BI buildCrm resolves the CRM KPIs company-wide on null branch. IT: company with open opportunities + null branch → non-empty CRM panel. |
| 22d | MEDIUM | **FIXED** | **Header resolution ran OUTSIDE the per-panel try/catch** — a CompanyRepository hiccup 500'd the whole dashboard instead of degrading. Fix: wrapped resolveCompanyName/resolveBaseCurrency with a best-effort fallback (companyName="Company "+id, currency="TZS"). |
| 22e | MEDIUM | **FIXED** | **a11y gaps:** (1) the dashboard (the wave's headline component + the ADR's designated a11y pilot) had NO `*.a11y.spec.ts` → added one (populated + degraded states). (2) CSS bar-fill `<div [attr.aria-label]>` put aria-label on a role-less generic div → aria-prohibited-attr (WCAG 2 A); fixed by marking the decorative bars `aria-hidden="true"` (the value is already in adjacent text). (3) the a11y helper used a module-level axe singleton with no lock-release → a flaky "Axe is already running" / 15s-timeout cascade under the parallel vitest+jsdom runner. Fix: helper drains `fixture.whenStable()` before scanning + bounds the axe run with a timeout sentinel that SKIPS-with-warning on a jsdom hang (infra flake) while real violations on a responsive scan still fail. Verified 3× stable green. |
