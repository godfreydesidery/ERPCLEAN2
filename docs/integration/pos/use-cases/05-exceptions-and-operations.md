...
- **Alternate / exception flows** (each is a real, separately-handled rejection from `PosSaleServiceImpl` / the shared handler — see [§09 §11](../09-sales-payments-receipts.md) and [§11 §2](../11-errors-offline-idempotency.md)):
  ...
  |---|-----------|--------|-------------------|------|
  | a | **Session not OPEN** | **409** (`ConflictException`) | `"POS session <uid> is not OPEN."` | The session is CLOSED/RECONCILED (or never opened). **Terminal — do not retry.** Open a fresh session (`POST /api/v1/pos/sessions`, [§08](../08-sessions.md)) and re-ring against the new `sessionUid`. See [UC-E5](#uc-e5-end-of-day-close-out-across-tills). |
...
- **Notes & limitations:**
  - The session-not-OPEN (a) and credit-limit (e) cases are **both 409 `IllegalStateException`/`ConflictException`** but are **terminal**, not transient. A client that retries every 409 will spam genuinely-rejected sales. Match the optimistic-lock message exactly before retrying ([§11 §2.1](../11-errors-offline-idempotency.md)).
...
- **Preconditions:** a sale POST was sent; the client received **no clean 201 and no clean terminal 4xx** — i.e. a socket timeout, dropped connection, app crash, or a `500`. The session uid is known; the client persisted a durable local transaction id for this basket *before* the first POST.
- **Main flow (the safe recovery procedure — [§11 §4.2](../11-errors-offline-idempotency.md)):**
  ...
  2. **Do NOT auto-retry.** There is **no idempotency key** — a blind resend mints a *second* finalised invoice and a *second* `SALE_FINALISED` event → duplicate stock issue + duplicate GL/AR ([§11 §4](../11-errors-offline-idempotency.md), [§12 #1](../12-known-limitations.md)). `X-Request-Id` does **not** dedupe.
  ...
  5. **Decide:**
     ...
     - **No match** → the sale did not commit; resend **once** (same basket, same local transaction id as `X-Request-Id` so a double-post is findable in server logs), then re-evaluate.
...
- **Notes & limitations:**
  - **No idempotency, by design today** ([§12 #1](../12-known-limitations.md)) — safe-retry responsibility is **entirely on the client**. No client cleverness fully closes the window between "server committed" and "client learned about it"; the reconcile discipline minimises it.
  - **No POS reversal/void** ([§12 #2](../12-known-limitations.md)): if you *do* double-post, there is no `/pos/sales/.../reverse`. The only POS recourse is a cash-drawer `REFUND` payout (which does **not** touch stock/GL/AR), leaving a back-office correcting journal ([§10](../10-returns-refunds.md)) — which is exactly why preventing the duplicate matters so much.
...
- **Alternate / exception flows:**
  - **Two opens on the same till** → second one gets **409** `"Till <tillUid> already has an OPEN session."` — a till may have **at most one OPEN session** ([§08 §1/§3](../08-sessions.md)). Close/reconcile the first, or use a different till.
...
- **Notes & limitations:**
  - **Cash-only, single exact tender** ([§12 #3](../12-known-limitations.md)): the POS path always adds **one** CASH payment in the header currency for exactly the gross — no split tender, no card/mobile money, no over-tender/change at the ledger.
...
- **Main flow (per till — [§08 §11](../08-sessions.md)):**
  ...
  5. **Repeat** for every open till. (To find them: `GET /api/v1/pos/sessions?companyId={id}&sort=openedAt,desc` ([§08 §5](../08-sessions.md)) and filter `status == OPEN`.)
- **Alternate / exception flows:**
  ...
  - **Sale arrives after close** → ringing against a CLOSED/RECONCILED session → **409** `"POS session <uid> is not OPEN."` ([UC-E1](#uc-e1-sale-rejected--the-five-ways-post-possales-says-no)); open a fresh session for the next shift.
...
- **Notes & limitations:**
  ...
  - A `REFUND` payout adjusts expected cash for reconciliation but is **not** a sale reversal ([§12 #2](../12-known-limitations.md), [§10](../10-returns-refunds.md)); merchandise returns at the POS are **not supported today** and must be handled office-side.
...
| UC-E2 Ambiguous outcome | `POST /pos/sales` → `GET /sales-invoices`, `GET .../sessions/uid/{uid}` | 500 / timeout, then 201 / 200 | [§11 §4](../11-errors-offline-idempotency.md), [§12 #1/#2](../12-known-limitations.md) |
...
> **Reality check.** Several desirable exception flows are **not supported today**: POS refund/void/reversal ([§12 #2](../12-known-limitations.md)), card/mobile-money/split tender ([§12 #3](../12-known-limitations.md)), manual price override ([§12 #4](../12-known-limitations.md)), server-side idempotent retry ([§12 #1](../12-known-limitations.md)), and offline/batch sale ingest ([§11 §4.3](../11-errors-offline-idempotency.md)). The workarounds above (reconcile-before-resend, cash `REFUND` payout + back-office correction, `lineDiscountAmount` for reductions) are the closest the current API allows.
