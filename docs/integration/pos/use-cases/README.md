...
| [UC-B9](./02-shift-lifecycle.md#uc-b9-refund--void-a-pos-sale-from-the-till--not-supported-today) | Refund / void a POS sale from the till | Shift Lifecycle | **no** | **§12 #2** |
| [UC-B10](./02-shift-lifecycle.md#uc-b10-non-cash-or-split-tender-at-the-till--not-supported-today) | Non-cash or split tender at the till | Shift Lifecycle | **no** | **§12 #3** |
...
| [UC-D1](./04-returns-and-refunds.md#uc-d1-customer-wants-to-return-goods--get-a-refund) | Customer wants to return goods / get a refund | Returns & Refunds | **no** | **§12 #2** |
| [UC-D2](./04-returns-and-refunds.md#uc-d2-correcting-a-mis-rung-sale-wrong-item-wrong-quantity-accidental-sale) | Correcting a mis-rung sale (wrong item / qty / accidental) | Returns & Refunds | **no** | **§12 #1/#2** |
...
> **Note on the four §12-bound scenarios.** Four "no" use cases are blocked by the same backend gaps catalogued in [§12 Known Limitations](../12-known-limitations.md), not by anything a client can configure: **UC-B9** and **UC-D1** (refund/void/return at the POS) hinge on [§12 #2 — no POS reversal/refund/void endpoint](../12-known-limitations.md#2-no-pos-reversal--refund--void-endpoint); **UC-D2** (correcting a mis-rung sale) compounds [§12 #1 — no idempotency](../12-known-limitations.md#1-no-idempotency-on-sale-creation--duplicate-posting-risk) with [§12 #2](../12-known-limitations.md#2-no-pos-reversal--refund--void-endpoint); and **UC-B10** (non-cash / split tender) hinges on [§12 #3 — single exact cash tender only](../12-known-limitations.md#3-single-exact-cash-tender-only). For all four, the documented workaround is a cash-drawer `REFUND` payout (drawer accuracy only) plus a back-office correcting entry — never treat them as POS-resolvable. (**UC-C9** — suspend/park & recall a sale — is also a **no**, but it is **not** a §12-bound scenario: it is simply **absent from the POS API** (there is no draft/hold/park endpoint to call), not a catalogued backend gap; its workaround is purely client-side basket state — see the UC-C9 notes. So the "four §12-bound" count stands.) (UC-C5 is marked **partial** for a different reason: `agentId` is accepted but not forwarded, so the invoice's agent defaults to the logged-in user — see the UC-C5 notes, not §12.) Group files also list a handful of *unnumbered* "not supported today" scenarios (e.g. update/move/reactivate a till, manual price override, sell on account) in their closing tables — read those for the full reality check.
...
| **Store manager / shift supervisor** | `POS.TILL.MANAGE`, `POS.SESSION.RECONCILE` (+ cashier codes); often `SALES.CREDIT.OVERRIDE` | UC-A3/A5, UC-B6, UC-E5 (reconcile), credit-override path in UC-C3/UC-E1 |
| **Cashier** | `POS.SALE.CREATE`, `POS.SESSION.OPEN`, `POS.SESSION.CLOSE`, `POS.SESSION.VIEW`, `POS.TILL.VIEW` (withhold `POS.TILL.MANAGE` + `POS.SESSION.RECONCILE`) | UC-B1–B5, UC-C1–C8, UC-D1/D2 (payout only), UC-E1–E5 |
| **Finance / back office** | `SALES.INVOICE.VIEW` + finance posting rights (outside the POS surface) | the mandatory correcting entries behind UC-D1/UC-D2 and any POS sale reversal |

**The seven `POS.*` codes** (full detail in [§01 §9](../01-authentication-and-permissions.md)): `POS.TILL.MANAGE` (create/deactivate tills), `POS.TILL.VIEW` (list/get tills), `POS.SESSION.OPEN` (open session **and** record payouts — there is no separate payout code), `POS.SESSION.CLOSE` (close/declare cash), `POS.SESSION.RECONCILE` (Z-read variance → GL; segregate from the cashier), `POS.SESSION.VIEW` (get/list/X-read), `POS.SALE.CREATE` (ring a sale).
