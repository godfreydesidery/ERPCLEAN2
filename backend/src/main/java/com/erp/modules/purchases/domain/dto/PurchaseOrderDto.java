 ...
 *
 * <p>{@code origin} (V96, K3) tells the UI whether this order was raised by a buyer or synthesised
 * to anchor a direct goods receipt, so a DIRECT_RECEIPT order can be badged when it does surface
 * (opened from its receipt, or when the list is asked to include them).
 */
public record PurchaseOrderDto(
    ...
    public static PurchaseOrderDto from(PurchaseOrder po, List<PurchaseOrderLineDto> lines) {
        return new PurchaseOrderDto(
                ...
                po.getOrderNumber(), po.getStatus(), po.getOrigin(),
