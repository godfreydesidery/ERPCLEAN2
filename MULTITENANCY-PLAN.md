...
- [ ] **P3-3** Org assert in `UserCompanyServiceImpl.assign`, `UserBranchServiceImpl.assign`,
      `UserRoleServiceImpl.assign`, and `UserServiceImpl.requireInScope` — **before** the membership
      oracle is consulted (`G1`, `G12`).
- [ ] **P3-4** Org predicate on `listOrgWide`; the derived query `findByRootFalseOrderByUsername`
      returns every non-root user in the database (`G1` step 1).
- [ ] **P3-5** Org predicate in `RoleServiceImpl.requireByUid` and `list()` (`G2`) — **NULL-tolerant
...
- [ ] **P3-6** Org assert in `CompanyServiceImpl.create`; gate `/companies/accessible`, currently
      `isAuthenticated()` only (`G3`).
- [ ] **P3-7** Rewrite `OrganisationServiceImpl.current()` and `list()` off the principal (`G5`).
- [ ] **P3-8** Audit the remaining "if root, drop the predicate" sites: `AuditReadService.java:57`,
      `CompanyServiceImpl.java:150`, `UserServiceImpl.java:166`/`:181`,
      `ProductStockReportQuery.java:393`.
...
- [ ] **P3-11** **The org check belongs inside `canActIn`, not only on the branch header.**
...
- [ ] **P3-12** **Re-triage the ArchUnit freeze store as a Phase 3 gate.**
